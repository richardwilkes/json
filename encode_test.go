...
type ctxMarshalKey int

type testMarshalContextStruct struct {
	Data string
}

func (t *testMarshalContextStruct) MarshalJSONWithContext(ctx context.Context) ([]byte, error) {
	v := ctx.Value(ctxMarshalKey(1))
	if v == nil {
		v = "?"
	}
	return json.Marshal(strings.SplitN(t.Data, "-", 2)[0] + "-" + v.(string))
}

func TestMarshalWithContext(t *testing.T) {
	ctx := context.WithValue(context.Background(), ctxMarshalKey(1), "22")
	testData := testMarshalContextStruct{Data: "hello"}
	data, err := MarshalWithContext(ctx, &testData)
	if err != nil {
		t.Fatal(err)
	}
	if string(data) != `"hello-22"` {
		t.Fatal("context did not pass data through: " + string(data))
	}
...
var _ Omitter = &testOmitterStruct{}

type testOmitterStruct struct {
	Data string
}

func (t testOmitterStruct) ShouldOmit() bool {
	return t.Data == "" || t.Data == "22"
}

func TestOmitter(t *testing.T) {
	type outer struct {
		Inner testOmitterStruct `json:",omitempty"`
	}
	var outerData outer
	data, err := Marshal(&outerData)
	if err != nil {
		t.Fatal(err)
	}
	if string(data) != "{}" {
	...
	outerData.Inner.Data = "x"
	data, err = Marshal(&outerData)
	if err != nil {
		t.Fatal(err)
	}
	if string(data) != `{"Inner":{"Data":"x"}}` {
		t.Fatal("unexpected result: " + string(data))
	}

	outerData.Inner.Data = "22"
	data, err = Marshal(&outerData)
	if err != nil {
		t.Fatal(err)
	}
