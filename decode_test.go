...
type ctxUnmarshalKey int

type testUnmarshalContextStruct struct {
	Data string
}

func (t *testUnmarshalContextStruct) UnmarshalJSONWithContext(ctx context.Context, in []byte) error {
	if err := Unmarshal(in, &t.Data); err != nil {
		return err
	}
	if s, ok := ctx.Value(ctxUnmarshalKey(1)).(string); ok {
		t.Data += "-" + s
	}
	return nil
}
...
func TestUnmarshalWithContext(t *testing.T) {
	ctx := context.WithValue(context.Background(), ctxUnmarshalKey(1), "22")
	var testData testUnmarshalContextStruct
	if err := UnmarshalWithContext(ctx, []byte(`"info"`), &testData); err != nil {
		t.Fatal(err)
	}
	if testData.Data != "info-22" {
		t.Fatal("context did not pass data through: " + testData.Data)
	}

	if err := Unmarshal([]byte(`"info"`), &testData); err != nil {
		t.Fatal(err)
	}
	if testData.Data != "info" {
